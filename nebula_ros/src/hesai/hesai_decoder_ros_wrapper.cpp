...
{
  double start_stamp_s = rclcpp::Time(this->get_clock()->now()).seconds();
  ...
  if (debug_publisher_) {
    ...
    double now_s = rclcpp::Time(this->get_clock()->now()).seconds();
    double received_stamp_s = rclcpp::Time(scan_msg->header.stamp).seconds();
